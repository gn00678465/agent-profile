...
| ------------ | ------------------ | ---------------------------------------- |
| Disk         | JSON / MD / JSONL  | `settings.json`, `CLAUDE.md`, `*.jsonl`  |
| Main Process | parsed JS objects  | `ConfigFile<ClaudeSettings>`             |
| IPC          | `IpcResponse<T>`   | `{ success: true, data: {...} }`         |
| Renderer     | React state        | unwrapped via `callElectron`             |
| UI           | Rendered output    | components, hooks                        |

### 3.1 Serialization across IPC (CRITICAL!)

Only JSON-serializable data crosses the IPC boundary — no functions, class instances, `Date` objects,
or circular refs. The agent config/session files own their own formats (e.g. session JSONL stores ISO
strings); read them as-is, pass plain serializable values over IPC, and format for display in the
renderer.

```typescript
// Keep IPC payloads plain & serializable; format at the UI edge
const iso = entry.timestamp;                     // string from the config/session file
<span>{new Date(iso).toLocaleString()}</span>;    // format only when rendering
```

**Rules**

1. **Serializable only** — non-serializable values silently drop or throw across IPC.
2. **Don't reshape on a whim** — these are real user files; preserve the format the CLI wrote.
3. **Format at the UI edge**, not in the main process.
