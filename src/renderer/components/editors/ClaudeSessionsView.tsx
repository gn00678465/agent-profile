...
import { FileText, Trash2, RefreshCw, FolderOpen, Search } from 'lucide-react';
...
export function ClaudeSessionsView({ configDir, agentColor }: ClaudeSessionsViewProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await callElectron(() =>
        electronAPI().config.getClaudeSessions(configDir)
      );
      setSessions(result);
    } catch (err) {
            ...
            >
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="mt-0.5 shrink-0">
                  <FileText className="h-4 w-4" style={{ color: agentColor }} />
                </div>
                {/* Content */}
